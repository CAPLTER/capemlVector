...
#' ) |>
#' {\(site) purrr::map(.x = site, ~ capemlVector::create_geographic_coverage(
#'   .x,
#'   description = .x$site
#' ))}() |>
#' unname()
...
create_geographic_coverage <- function(sf_object, description) {
  ...
  if (!inherits(sf_object, "sf"))  { stop("function requires an sf object") }
  if (nrow(sf_object) == 0)        { stop("sf_object is empty") }
  if (!all(sf::st_is_simple(sf_object))) { stop("sf_object contains non-simple geometries") }
